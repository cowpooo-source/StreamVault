function createOperationalMetrics({ now = Date.now, windowMs = 60_000 } = {}) {
  ...
  function finish(type = 'other', status = 200, duration = 0) {
    activeRequests = Math.max(0, activeRequests - 1);
    const timestamp = now();
    cleanup(timestamp);
    const second = Math.floor(timestamp / 1000);
    const bucket = buckets.get(second) || { total: 0, errors: 0, latency: 0, byType: {}, byStatus: {} };
    bucket.total += 1;
  ...
  function snapshot() {
    cleanup();
    const summary = { total: 0, errors: 0, latency: 0, byType: {}, byStatus: {} };
