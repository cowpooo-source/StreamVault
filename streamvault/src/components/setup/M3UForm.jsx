...
export function M3UForm({ form, setForm, rawText, setRawText, detected, selected, setSelected, onSubmit, onFileImport, onImportMultiple, onDetect }) {
  ...
  return (
    <>
      <div className="fg">
        <label className="fl">Playlist URL</label>
        <input
          className="fi"
        ...
        />
        <div className="fhint">Supports .m3u and .m3u8 playlist files</div>
      </div>
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: ".5rem", padding: ".45rem .65rem",
                ...
                onClick={() => {
                  if (detected.length > 1) {
                    ...
                    // Single detection: notify parent via onDetect callback for single item
                    if (onDetect) onDetect(d, true);
                  }
                }}
